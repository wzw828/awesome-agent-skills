...
作为定制 AI 能力最强大方式之一，Skill 能与 AI 的内置能力（如代码执行和文档创建）协同良好。

对于构建和使用 MCP 的用户，它还提供了另一个强大层级的功能——帮助 AI 将外部工具访问转化为可靠、优化的工作流程。

## 增强插件

在官方 skill-creator skill 基础上，本项目增加了 [Claude Skill 完整构建指南](docs/Claude-Skills-完全构建指南.md) 中的最佳实践，构建了一个基于 Claude Code Plugin 机制的 Agent Skills Toolkit ，帮助你快速创建和改进 Agent Skills。
